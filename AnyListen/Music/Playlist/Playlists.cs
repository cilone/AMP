...
namespace AnyListen.Music.Playlist
{
    ...
    public static class Playlists
    {
        ...
        public static IEnumerable<PlayableBase> ImportFiles(IEnumerable<string> paths, EventHandler<TrackImportProgressChangedEventArgs> progress)
        {
            ...
            foreach (var fi in enumerable.Select(x => new FileInfo(x)))
            {
                if (fi.Exists && RightExtension.Contains(fi.Extension.ToLower()))
                {
                    progress?.Invoke(null, new TrackImportProgressChangedEventArgs(index, count, fi.Name));
