...
namespace AnyListen.Music.Visualization
{
    class SampleAnalyser
    {
        ...
        public void Add(float[] samples)
        {
            ...
            {
                var s = MergeSamples(samples, i, channels);
        ...
        {
            var input = new Complex[_storedSamples.Length];
        ...
        private void UpdatePeaks(float[] samples, int index, int channelCount)
        {
            ...
            if (channelCount > 2)
            {
                for (var j = 2; j <= channelCount - 1; j++)
                {
        ...
        private float MergeSamples(float[] samples, int index, int channelCount)
        {
            ...
            var z = 0f;
            for (var i = 0; i <= channelCount - 1; i++)
            {
                z += samples[index + i];
            }
            return z / 2f;
